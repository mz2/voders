...
def extract_monophonic_notes(
    ...
    if onset_threhold_type == "fixed":
        ...
        if not HAS_CV2:
            raise ImportError(
                "opencv-python (cv2) is required for adaptive-otsu thresholding. Install with: pip install opencv-python"
            )
        ...
        if not HAS_CV2:
            raise ImportError(
                "opencv-python (cv2) is required for adaptive-mean thresholding. Install with: pip install opencv-python"
            )
        block_size = 101
        cv2_onset_probs = (onset_probs * 255).astype(np.uint8)[:, np.newaxis]
        ...
        if not HAS_CV2:
            raise ImportError(
                "opencv-python (cv2) is required for adaptive-gaussian thresholding. Install with: pip install opencv-python"
            )
        block_size = 101
        cv2_onset_probs = (onset_probs * 255).astype(np.uint8)[:, np.newaxis]
        onset_probs_mask = cv2.adaptiveThreshold(
    ...
    while onset_idx_ptr < len(onset_indices):
        ...
        while onset_idx_ptr < len(onset_indices) and onset_indices[onset_idx_ptr] < current_frame:
            onset_idx_ptr += 1
        ...
        if segment_start < segment_end:  # Ensure the segment has duration
            ...
            segment_pitch_classes = np.argmax(pitch_class_probs[segment_start:segment_end], axis=1)
