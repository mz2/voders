...
class BasicPitchTorch(nn.Module):
    def __init__(
        ...
        if stack_harmonics:
            self.hs = HarmonicStacking(
                ...
                n_output_freqs=BASIC_PITCH_N_SEMITONES * BASIC_PITCH_CONTOURS_BINS_PER_SEMITONE,
            )
        ...
        n_semitones = int(
            np.min(
                [
                    int(np.ceil(12.0 * np.log2(len(stack_harmonics))) + BASIC_PITCH_N_SEMITONES),
                    BASIC_PITCH_MAX_N_SEMITONES,
                ]
            )
        )
        ...
        self.conv_contour = nn.Sequential(
            nn.Conv2d(
                num_in_channels, CONV_CONTOUR_CHANNELS, kernel_size=(3, 3 * 13), padding="same"
            ),
    ...
    def forward(self, x: Tensor) -> dict[str, Tensor]:
        if x.dim() == 3 and x.shape[1] == 1:
            ...
            raise ValueError(
                f"Expected audio shape (batch, time) or (batch, 1, time), got {x.shape}"
            )
        ...
        note = align_to_label_frames(piano_logits_to_midi_layout(x_note.squeeze(1)), n_samples)
        onset = align_to_label_frames(piano_logits_to_midi_layout(x_onset.squeeze(1)), n_samples)
