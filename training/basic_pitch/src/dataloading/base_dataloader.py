...
class PianoRollAudioDataset(Dataset):
            ...
            for input_files in tqdm(self.files(group), desc=f"loading group {group}"):
                ...
                if self.sequence_length is not None:
                    ...
                    if full_audio_length <= self.sequence_length:
                        ...
                        for chunk_start in range(0, full_audio_length, self.sequence_length):
                            chunk_starts.add(
                                min(
                                    chunk_start,
                    ...
                    for chunk_start in sorted(chunk_starts):
                        label = self._load_labels_chunk(tsv_path, chunk_start, self.sequence_length)
    ...
    def __getitem__(self, index):
        ...
        if self.sequence_length is not None:
            ...
            label = self._load_labels_chunk(data["tsv_path"], chunk_start, audio.shape[1])
            expected_frames = (audio.shape[1] - 1) // HOP_LENGTH + 1
            if label.shape[0] < expected_frames:
                label = torch.nn.functional.pad(label, (0, 0, 0, expected_frames - label.shape[0]))
    ...
    def _load_tsv_cached(self, tsv_path):
        ...
        if tsv_key not in self._tsv_cache:
            # Klangio annotations have a comment header while voders annotations
            # are headerless. np.loadtxt ignores the former without dropping the
            # first note from the latter.
            midi = np.loadtxt(tsv_path, delimiter="\t", comments="#")
            if midi.ndim == 1:
    ...
    def _load_audio(self, audio_path, offset=0, num_frames=-1):
        ...
        if num_frames > 0:
            if audio.shape[1] > num_frames:
                ...
                audio = torch.nn.functional.pad(audio, (0, num_frames - audio.shape[1]))
