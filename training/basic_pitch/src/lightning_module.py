...
class LightningModuleSingingVoice(pl.LightningModule):
    def __init__(
        ...
        self.frame_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(frame_weight))
        self.contour_loss = nn.BCEWithLogitsLoss()
        self.onset_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(onset_weight))
    ...
    def _log_training_media(self, batch, model_outputs, step: int) -> None:
        ...
        if self.logger is None or not getattr(self.trainer, "is_global_zero", True):
            return
        if not all(
            hasattr(self.logger, method) for method in ("log_audio", "log_image", "log_table")
        ):
            return
    ...
    def _forward_with_context(
        ...
        for key, value in outputs.items():
            if value.dim() >= 2 and value.shape[1] >= 2 * context_frames:
                stripped[key] = value[:, context_frames : context_frames + center_frames]
            else:
    ...
    def _build_contour_targets(
        ...
        contour = piano_frames.repeat_interleave(BASIC_PITCH_CONTOURS_BINS_PER_SEMITONE, dim=-1)
    ...
    def compute_contour_loss(
        ...
        contour_targets = self._build_contour_targets(frame_labels, contour_logits.shape[1])
    ...
    def training_step(self, batch, batch_idx):
        ...
        model_outputs = self._forward_with_context(batch["audio"], return_contour=True)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        for loss_key, loss_val in loss_dict.items():
            self.log(
                f"eval/{loss_key}", loss_val, prog_bar=True, batch_size=batch["audio"].shape[0]
            )
        ...
        example_data.update(
            {
                "predicted_onsets": torch.sigmoid(model_outputs["onset"][0, ...])
                .detach()
                .float()
                .cpu()
                .numpy(),
                "predicted_frames": torch.sigmoid(model_outputs["frame"][0, ...])
                .detach()
                .float()
                .cpu()
                .numpy(),
    ...
    def on_validation_epoch_end(self):
        ...
        for onset_thresh in np.linspace(0.1, 0.9, 3):  # Wow! You are really curious about the code!
            for frame_thresh in np.linspace(
                0.1, 0.9, 3
            ):  # Do not be shy to mess around these values
                for example in (
                    self.examples_validation
                ):  # But mind that the more threshold combinations that you explore
                    predicted_notes = (
                        output_to_notes_polyphonic(  # The more time each validation epoch will take
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        model_outputs = self._framewise_inference_on_batches(batch["audio"])
        assert model_outputs["onset"].shape[0] == 1, (
            "For prediction, batch size must be of size one"
        )
    ...
    def _framewise_inference_on_batches(self, x_batch):
        ...
        for audio_chunk_batch in chunk_batches:
            if profiling and not self._nsys_capture_complete and self._nsys_step == warmup_steps:
            ...
            capture_this_step = (
                self._nsys_capture_started and self._nsys_captured_steps < capture_steps
            )
            if capture_this_step:
                torch.cuda.nvtx.range_push(f"final_inference/step_{self._nsys_captured_steps}")
            try:
