...
- Q: What is the minimum note-duration cutoff for flag/reject? → A: It is not a fixed constant — it is **learned** by an analysis pass over the input annotation data. Before rendering, the system computes the distribution of note durations across the supplied score set and derives `min_note_ms` from a low percentile of that distribution (default: 1st percentile), never below the validator's onset tolerance (50 ms) as a hard sanity floor. The learned value is recorded in the run's stats/manifest for auditability. The earlier "~40 ms" and "50 ms × 2" mentions are illustrative, not normative.
- Q: Should the learned thresholds also account for the methods used? → A: Yes, via two documented numbers per method (not invented constants): its analysis frame hop (e.g., CREPE = 10 ms) and any constant group delay (e.g., a codec/resampler's published latency). Effective `min_note_ms` = max(data-derived percentile, largest active frame hop, 50 ms onset tolerance); and the validator subtracts a method's constant group delay before comparing measured onsets to the score. Values come from each method's documentation or a one-time measurement.
...
3. **Given** a score containing a note shorter than the learned `min_note_ms` threshold (learned from the input annotation distribution and the active methods' timing resolution), **When** the pipeline renders it, **Then** the sample is either accepted with a flag or rejected with a clear reason logged to provenance.
...
- **Sub-tolerance note duration**: Notes shorter than the `min_note_ms` threshold cannot be reliably bounded by the onset tolerance; the system flags or rejects them rather than silently emitting unlearnable samples. `min_note_ms` is **learned** as max(a low percentile — default 1st — of the input scores' note-duration distribution, the temporal-resolution floor of the active pipeline methods, the 50 ms onset tolerance), and the learned value plus the contributing method floors are recorded for audit.
...
- **FR-018**: Before rendering, the system MUST run an analysis pass over the input score set to learn the `min_note_ms` threshold. The learned value MUST be `max(low percentile (default 1st) of the note-duration distribution, the temporal-resolution floor of the active pipeline methods, the 50 ms onset tolerance)`, and the learned value plus the contributing method floors MUST be recorded in the run's stats/manifest. The validator then flags or rejects notes shorter than this learned threshold.
- **FR-019**: Each timing-affecting method (f0 estimator, onset detector, forced aligner, codec/resampler) MUST contribute two values taken from its own documentation (or a one-time measurement) — its analysis frame hop and any constant group delay. The system MUST (a) floor the FR-018 `min_note_ms` at the largest active frame hop, and (b) subtract a method's constant group delay from measured onsets/offsets before the validator compares them to the score, so a fixed delay (e.g., codec/resampler) does not cause spurious rejections. No timing constants are invented; unknown values are measured, not guessed.
...
- **SC-009**: Any published corpus can be regenerated end-to-end from its manifest plus the source scores within this documented tolerance, demonstrating reproducibility:
  - **Deterministic and voice-conversion lanes** (CPU, or GPU with fixed seed and `auto_predict_f0=False`): the regenerated audio is **bit-exact** to the original on the same platform and pinned library versions.
  - **Neural-SVS / other non-bit-deterministic GPU lanes**: the regenerated sample MUST (a) receive the **same `ValidationVerdict.status`** as the original, and (b) match the original within the validator's own tolerances — every note's measured f0 within ±25 cents over ≥80% of the sustained interval and every onset within max(10 ms, the f0/onset estimator's frame resolution) of the original render's onset.
  Reproducibility is verified by re-rendering a random sampled subset of manifest records in isolation (by their derived seeds) and asserting the above per lane.
